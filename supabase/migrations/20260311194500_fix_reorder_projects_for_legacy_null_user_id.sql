/*
  # Allow project reordering for legacy rows with NULL user_id

  Older project rows may still have a NULL user_id. The admin UI can manage
  those rows through existing RLS policies, so the reorder RPC must allow the
  same access pattern.
*/
...
begin
  ...
  where (project.user_id = auth.uid() or project.user_id is null)
  ...
  where project.id = ordered_projects.project_id
    and (project.user_id = auth.uid() or project.user_id is null)
    and public.project_primary_category(project.category) = normalized_category_key;
end;
$function$;
